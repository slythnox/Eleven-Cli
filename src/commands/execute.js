...
export async function handleExecute(query, options = {}) {
  ...
  try {
    console.log(chalk.blue('🤖 Forge CLI - AI Command Assistant'));
    ...
    if (options.planOnly) {
      console.log(chalk.gray('\nPlan-only mode: execution skipped'));
      return;
    }
    ...
    if (!options.autoApprove) {
      ...
      if (stepsRequiringConfirmation.length > 0 || highRiskSteps.length > 0) {
        console.log(chalk.yellow('\n⚠️  The following steps require confirmation:'));
        ...
        const { proceed } = await prompts({
          type: 'confirm',
          name: 'proceed',
          message: 'Do you want to proceed with execution?',
          initial: false
        });

        if (!proceed) {
          console.log(chalk.gray('Execution cancelled by user'));
          return;
        }
      }
    }
    ...
    for (const step of plan.steps) {
      const stepSpinner = ora(`Executing: ${step.description}`).start();
    ...
    console.log(`Successful: ${executionResults.filter(r => r.success).length}`);
    console.log(`Failed: ${executionResults.filter(r => !r.success).length}`);
    console.log(`Overall status: ${allSuccessful ? chalk.green('SUCCESS') : chalk.red('PARTIAL/FAILED')}`);
