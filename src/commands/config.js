...
async function handleConfig(options = {}) {
  try {
    if (options.show) {
      ...
      console.log(`${chalk.cyan('API Rotation:')} ${config.useRotation ? '✅ Enabled' : '❌ Disabled'}`);
    ...
    if (options.rotation) {
      ...
      console.log(chalk.green(`✅ API rotation ${config.useRotation ? 'enabled' : 'disabled'}!`));
      if (config.useRotation && (!config.apiKeys || config.apiKeys.length < 2)) {
        console.log(chalk.yellow('⚠️  Add more API keys for effective rotation: el config -a YOUR_SECOND_KEY'));
      }
      return;
    }
