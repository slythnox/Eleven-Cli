...
class GeminiService {
  ...
  initialize() {
    ...
    if (!apiKey) {
      throw new Error(
        "API key not found. Please set it using: el config -k YOUR_API_KEY"
      );
    }
  ...
  checkRateLimit() {
    ...
    if (this.requestCount >= 14) {
      ...
      if (config.useRotation && config.apiKeys && config.apiKeys.length > 1) {
        console.log(
          chalk.yellow("🔄 Rotating API key to avoid rate limits...")
        );
        ...
        throw new Error(
          "Rate limit approaching. Consider adding more API keys for rotation."
        );
      }
    }
  ...
  async retryWithBackoff(operation, attempt = 1) {
    try {
      ...
      console.log(
        chalk.yellow(
          `⏳ Retrying in ${backoffTime / 1000}s... (attempt ${attempt}/${
            this.maxRetries
          })`
        )
      );
  ...
  handleError(error) {
    if (
      ...
      throw new Error(
        "Invalid API key. Please check your Gemini API key with: el config -k YOUR_KEY"
      );
    }
    ...
    if (
      ...
      throw new Error(
        "Rate limit exceeded. Add more API keys: el config -a YOUR_SECOND_KEY"
      );
    }
    ...
    if (error.message.includes("SAFETY")) {
      throw new Error(
        "Content was blocked for safety reasons. Please rephrase your request."
      );
    }
