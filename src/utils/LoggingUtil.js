  ...
  async getExecutionStats() {
    ...
    try {
      ...
      const stats = {
        ...
        failed: executions.filter(e => e.status === 'partial_failure' || e.status === 'failed').length,
        averageSteps: 0,
        ...
        recentActivity: executions.slice(0, 5).map(e => ({
          ...
          query: e.userQuery,
          status: e.status,
          stepCount: e.executionResults?.length || 0
        }))
      };

      if (executions.length > 0) {
        const totalSteps = executions.reduce((sum, e) => sum + (e.executionResults?.length || 0), 0);
        stats.averageSteps = Math.round(totalSteps / executions.length * 100) / 100;

        executions.forEach(e => {
          const riskLevel = e.validationResult?.riskLevel || 'low';
      ...
      this.error('Failed to get execution stats', { error: error.message });
      return {
        total: 0,
        successful: 0,
        failed: 0,
